# install_batch2b.ps1 -- full site app: login, attendance, worker + Aadhaar,
#   daily entry (progress / petty cash / holiday). Cost figures are never shown.
...
<div id="appShell" class="hidden">
  ...
  <div class="wrap">
    ...
    <section id="homeScreen" class="hidden">
      ...
      <div class="home-grid">
        <button class="tile" data-go="attend"><span class="ic">◉</span>Attendance</button>
        <button class="tile" data-go="daily"><span class="ic">✎</span>Daily entry</button>
        <button class="tile" data-accent="brand" data-go="enroll"><span class="ic">＋</span>Add worker</button>
        <button class="tile" data-go="today"><span class="ic">≡</span>Who's in</button>
      </div>
    </section>
...
(function () {
  ...
  function doLogout(){
    fetch("/api/method/logout").then(function(){location.reload();});
  }
...
foreach ($t in $targets) {
    ...
    Write-Host ("  wrote " + $t.path + "  (" + $b.Length + " bytes)") -ForegroundColor Green
}

# verify the new endpoints are present in site_api.py
$api = Join-Path $app "sbi_projects\site_api.py"
foreach ($fn in @("save_aadhaar","submit_daily_log","get_app_menu","get_progress_parameters")) {
    if (Select-String -Path $api -Pattern ("def " + $fn) -Quiet) { Write-Host ("  ok  " + $fn) -ForegroundColor Green }
    else { Write-Error ("missing endpoint: " + $fn); exit 1 }
}
...
git add -A -- $app
git commit -m "feat: full site app - login, attendance, worker+aadhaar OCR, daily entry"
git push origin main
Write-Host "Pushed. Deploy once, then open /site_app on the tablet." -ForegroundColor Green
