...
Site staff (Projects User) enter daily actuals here:
expenses (equipment / SC / transport / other), progress
(day-wise + labour-wise) and material issue. No budget or
money aggregates are ever returned to site users (Q3 rule).
"""
...
def get_bootstrap(project):
    ...
    if boq:
        items = frappe.get_all("Project BOQ Item", filters={"parent": boq},
                               fields=["stage", "activity", "unit", "qty"],
                               order_by="idx")
        smap = {}
        for it in items:
            smap.setdefault(it.stage, []).append(
                {"activity": it.activity, "unit": it.unit, "qty": it.qty})
        stages = [{"stage": k, "activities": v} for k, v in smap.items()]
    else:
        tasks = frappe.get_all("Task",
                               filters={"project": project, "is_group": 1},
                               fields=["subject"], order_by="creation")
        stages = [{"stage": t.subject, "activities": []} for t in tasks]
    ...
    return {"project": project, "project_name": pname, "stages": stages,
            "workers": workers,
...
@frappe.whitelist()
def add_progress(project, entry_date, stage, qty_done=None,
...
@frappe.whitelist()
def material_issue(project, warehouse, item_code, qty, stage=None):
...
def list_today(project, entry_date=None):
    """Entries made today - shown back for confirmation. No cost totals
    beyond what the site user themselves typed."""
    _check_site_user()
