...
Site masters
------------
Every Project (= a construction site) provisions its own Cost Center tree and
Warehouse.  The site Cost Center is a GROUP holding one leaf per cost category
(General, Material, Manpower, Equipment Cost, Equipment Rent, Subcontracting,
Overheads).  Postings always target a leaf -- income, tax and anything
un-categorised land on the "General" leaf, which is what Project.cost_center
points at.  Assets and retention are booked to their own balance-sheet accounts
and merely tagged with the site cost center (standard ERPNext), so they are not
category leaves here.
"""
...
# Category leaves created under every site cost center group.
# (label, is_default)  -- exactly one row is the default/General leaf that
# Project.cost_center is set to, because you cannot post to a group.
...
# ---------------------------------------------------------------------------
# Site masters: auto-create Cost Center tree + Warehouse for every Project
# ---------------------------------------------------------------------------

def create_site_masters(doc, method=None):
    """Project.after_insert -- provision site Cost Center tree and Warehouse.
...
def _provision_site_masters(doc):
    ...
    # cc = the General (default) leaf, because you cannot post to a group
...
def _ensure_site_cost_center(company, abbr, label):
    """Create the site as a GROUP cost center with one leaf per cost category.

    Returns the *General* leaf name (the postable default for income / tax).
    A pre-existing leaf from before this structure is left untouched (the
    backfill step converts those); its name is returned as-is so nothing breaks.
    """
    group_name = "{0} - {1}".format(label, abbr)
    existing = frappe.db.get_value(
        "Cost Center", group_name, ["name", "is_group"], as_dict=True
    )

    # legacy leaf (created before category structure) -> leave for backfill
    ...
    return _ensure_category_leaves(company, abbr, label, group_name)


def _ensure_category_leaves(company, abbr, label, group_name):
    """Create the category leaves under a site group; return the General leaf."""
    general_leaf = None
    for cat_label, is_default in SITE_COST_CATEGORIES:
        display = "{0} {1}".format(label, cat_label)[:125]
        leaf_name = "{0} - {1}".format(display, abbr)

        if not frappe.db.exists("Cost Center", leaf_name):
            frappe.get_doc({
                "doctype": "Cost Center",
                "cost_center_name": display,
                "company": company,
                "parent_cost_center": group_name,
                "is_group": 0,
            }).insert(ignore_permissions=True)

        if is_default:
            general_leaf = leaf_name

    return general_leaf or group_name
...
def backfill_site_masters(project=None):
    ...
    NOTE: this only (re)provisions missing masters. Converting an OLD single-leaf
    site cost center into the new group+categories structure is handled by the
    dedicated backfill script (Step 2), not here.
    """
