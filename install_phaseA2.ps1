# install_phaseA2.ps1 -- Phase A complete: CSRF fix (attendance now works),
#   sound + vibration on punch, on-site GPS status, install button, green face ring,
#   geo-fence boundary set + out-of-bounds report.
...
<div id="appShell" class="hidden">
  <div class="top">
    ...
    <button id="btnInstall" type="button" class="hidden">Install</button>
  ...
  <div class="wrap">
    ...
    <section id="attendScreen" class="hidden">
      ...
      <div class="note" id="gpsNote" data-tone="">Checking your location…</div>
...
(function () {
  ...
  function beep(kind){
    try{
      ...
      // ok = two rising notes, warn = one low, err = low buzz
      var freqs = kind==="ok"?[660,880]:kind==="warn"?[440]:[220];
      ...
      g.gain.exponentialRampToValueAtTime(0.0001,t+freqs.length*0.12+0.05);
      o.start(t); o.stop(t+freqs.length*0.12+0.08);
    }catch(e){}
    if(navigator.vibrate){
      navigator.vibrate(kind==="ok"?[60,40,60]:kind==="warn"?[120]:[200,80,200]);
    }
  }
  ...
  function watchGps(){
    ...
    navigator.geolocation.watchPosition(
      function(p){S.gps={lat:p.coords.latitude,lng:p.coords.longitude};paintGps();},
      function(){S.gps=null;},
  ...
  // -------- gps geofence status --------
  var siteFence=null;  // {lat,lng,radius} for the current project
  function loadFence(){
    return call("site_api.get_site_fence",{project:S.project}).then(function(f){
      siteFence=f&&f.latitude?f:null; paintGps();
    }).catch(function(){siteFence=null;});
  }
  ...
  function paintGps(){
    var el=$("gpsNote"); if(!el)return;
    if(!siteFence){note(el,"","Site boundary not set. Attendance is recorded without a location check.");return;}
    if(!S.gps){note(el,"warn","Waiting for your location…");return;}
    var d=haversine(siteFence.latitude,siteFence.longitude,S.gps.lat,S.gps.lng);
    if(d<=siteFence.radius){note(el,"ok","You are on site ("+Math.round(d)+" m from centre).");}
    else{note(el,"warn","You are "+Math.round(d)+" m away — outside the site boundary.");}
  }
  ...
  function freezeEnroll(){
    detect($("video2")).then(function(res){
      ...
      $("stage2").setAttribute("data-face","yes");   // keep the green ring on the frozen shot
      enFrozen=true;$("btnFreeze2").textContent="Retake";$("who2").textContent="✓ Face captured";
      beep("ok");
  ...
  function showScreen(which){
    ...
    if(which==="attend"){startCamera($("video"),S.facing,"att").catch(function(){});loadFence();}
    else{stopCamera("att");}
  ...
  function setupInstall(){
    ...
    window.addEventListener("beforeinstallprompt",function(e){
      e.preventDefault();deferred=e;$("btnInstall").classList.remove("hidden");});
    $("btnInstall").onclick=function(){
      if(!deferred)return;deferred.prompt();
      deferred.userChoice.then(function(){deferred=null;$("btnInstall").classList.add("hidden");});};
    window.addEventListener("appinstalled",function(){$("btnInstall").classList.add("hidden");});
  }
...
foreach ($fn in @("save_aadhaar","submit_daily_log","get_site_fence")) {
...
git add -A -- $app
git commit -m "fix: CSRF token for attendance; add sound/vibration, GPS status, install, geofence"
git push origin main
Write-Host "Pushed. Deploy once. On the tablet: unregister the old service worker, then reload." -ForegroundColor Green
